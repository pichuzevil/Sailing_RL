...
class MyAgent(BaseAgent):
    def __init__(self):
        ...
        self.safety_buffer = 2.5 # Padding for inertia/jitter
        self.world_map = None
        self.planned_path = []
        self.replan_freq = 8 # Re-sync with environment every 8 steps
    ...
    def plan_path(self, start_pos, start_vel, start_wind):
        ...
        while pq and nodes_explored < max_nodes:
            ...
            for i, a_dir in enumerate(self.action_dirs):
                # 1. OPTIMIZATION: Ignore 'Stay' or 'Backward' actions if speed is the goal
                if i == 8: continue # 'Stay' is never optimal for speed
                
                # 2. VMG Check: If the wind is North and we move South, skip unless blocked
                if a_dir[1] < 0 and y < 100: continue 

                eff = self._get_efficiency(a_dir, current_wind)
                v_target = eff * self.max_speed * a_dir
                v_next = self.inertia_factor * np.array([vx, vy]) + (1 - self.inertia_factor) * v_target
                
                v_disc = np.where(v_next < 0, np.ceil(v_next), np.floor(v_next)).astype(np.int32)
                new_pos = curr_pos + v_disc

                # 3. Trajectory Safety (Use the 5-point check)
                if not self._is_path_safe(curr_pos, new_pos): continue

                # 4. GOLD HEURISTIC: Focus on Y-Progress (The Goal is at Y=127)
                # We weight vertical progress higher than horizontal alignment
                y_remaining = 127 - new_pos[1]
                x_alignment = abs(64 - new_pos[0])
                
                # The 'Cost' of time (t) + the 'Potential' of remaining distance
                h = (y_remaining / (self.max_speed * 0.7)) + (x_alignment / self.max_speed)
